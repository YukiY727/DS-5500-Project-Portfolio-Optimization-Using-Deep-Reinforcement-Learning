...
class PolicyCNN(object):
    ...
    def __init__(self, ohlc_feature_num, ticker_num, num_trading_periods, sess, optimizer, trading_cost, cash_bias_init, interest_rate, 
        ...
        self.convolution = self.convolution_layers(self.X_t, self.num_filter_layer_1, self.kernel_size, self.num_filter_layer_2, self.num_trading_periods) 
        self.action_chosen = self.policy_output(self.convolution, self.cash_bias)
        self.adjusted_reward = self.reward(shape_X_t, self.action_chosen, self.interest_rate, self.weights_previous_t, self.pf_previous_t, self.daily_returns_t, self.trading_cost)

    def convolution_layers(self, X_t, num_filter_layer_1, kernel_size, num_filter_layer_2, num_trading_periods):
        ...
        with tf.compat.v1.variable_scope("Convolution3"):
            convolution3 = tf.compat.v1.layers.conv2d(
                inputs=convolution2,
    ...
    def reward(self, shape_X_t, action_chosen, interest_rate, weights_previous_t, pf_previous_t, daily_returns_t, trading_cost):
        ...
        with tf.compat.v1.variable_scope("Reward"):
            ...
            zero_vector = tf.tile(tf.constant(np.array([0.0] * self.ticker_num).reshape(1, self.ticker_num), shape=[1, self.ticker_num], dtype=tf.float32), tf.stack([shape_X_t, 1]))
        ...
        with tf.compat.v1.variable_scope("Reward-adjusted"):
            adjusted_reward = instantaneous_reward - instantaneous_reward_eq - self.adjusted_rewards_alpha * tf.reduce_max(action_chosen)
