...
def max_drawdown(
    weights, time_period=num_trading_periods, portfolio_data_frame=pivot_table_crypto
):
...
def sharpe_crypto(
    w, returns_cov=crypto_covariance_matrix, returns_mean=crypto_returns_mean
):
    w = w.reshape(
        len(
            w[0],
...
def main(stocks=True):
    ...
    plot_wts_assigned(
        weight_vectors[-1], ticker_num, ticker_list, "wt_vector_crypto.png"
    )
