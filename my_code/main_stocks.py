...
def max_drawdown(
    weights, time_period=num_trading_periods, portfolio_data_frame=pivot_table_stocks
):
    ...
    for i in range(int(len(simulated_portfolio) / time_period) - 1):
        if min(simulated_portfolio[i * time_period : (i + 1) * time_period]) > 0:
            biggest_variation = max(
                simulated_portfolio[i * time_period : (i + 1) * time_period]
            ) / min(simulated_portfolio[i * time_period : (i + 1) * time_period])
        else:
            biggest_variation = 0
...
def RoMad(
    weights, mdd, returns_mean=stocks_returns_mean, time_period=num_trading_periods
):
    weights = weights.reshape(
        len(
            weights[0],
...
def sharpe_stocks(
    w, returns_cov=stocks_covariance_matrix, returns_mean=stocks_returns_mean
):
    w = w.reshape(
        len(
            w[0],
    ...
    portfolio_volatility = np.sqrt(
        np.dot(w.T, np.dot(returns_cov.values, w)) * nbr_trading_days
    )
...
def main(stocks=True):
    ...
    env_pf_optimizer = RLEnv(
        Path=data_path,
        PortfolioValue=portfolio_value_init,
        TransCost=trading_cost,
        ReturnRate=interest_rate,
        WindowSize=num_trading_periods,
        TrainTestSplit=train_data_ratio,
    )

    # environment set up for the portfolio equiweight agent
    env_pf_equiweight = RLEnv(
        Path=data_path,
        PortfolioValue=portfolio_value_init,
        TransCost=trading_cost,
        ReturnRate=interest_rate,
    ...
    with tf.compat.v1.Session() as sess:
        ...
        checkpoint = tf.train.Checkpoint(optimizer=optimizer, model=pf_opt_agent)
        train_pf_values = []
            ...
            for batch_num in tqdm.tnrange(num_batches, desc="Batches"):
                ...
                while training_batch_t_selection == False:
                    training_batch_t = (
                        ...
                        - training_batch_size
                        + 1
                        - np.random.geometric(p=beta_pvm)
                    )
                    if training_batch_t >= 0:
                        training_batch_t_selection = True

                state, done = env_pf_optimizer.ResetEnvironment(
                    pvm.get_wt_vector_t(int(training_batch_t)),
                    portfolio_value_init,
                    training_batch_t,
                )
                state_equiweight, done_equiweight = env_pf_equiweight.ResetEnvironment(
                    equiweight_vector, portfolio_value_init, training_batch_t
                )
                ...
                for training_batch_num in tqdm.tnrange(
                    ...
                    action = (
                        pf_opt_agent.compute_weights(X_t, Wt_previous)
                        if (np.random.rand() < epsilon)
                        else get_random_action(ticker_num)
                    )
    ...
    state, done = env_pf_optimizer.ResetEnvironment(
        weight_vector_init_test, portfolio_value_init_test, training_batch_t
    )
    state_equiweight, done_equiweight = env_pf_equiweight.ResetEnvironment(
        equiweight_vector, portfolio_value_init_test, training_batch_t
    )
    ...
    start_step_num = int(
        training_steps + validation_steps - int(num_trading_periods / 2)
    )
    end_step_num = int(
        training_steps + validation_steps + test_steps - num_trading_periods
    )
    ...
    for test_step_num in range(start_step_num, end_step_num):
        ...
        state, reward, done = env_pf_optimizer.Step(action)
        state_equiweight, reward_equiweight, done_equiweight = env_pf_equiweight.Step(
            equiweight_vector
        )
        ...
        sharpe_ratio = round(sharpe_stocks(w=wt_previous), 3)
        mdd = round(max_drawdown(weights=Wt_previous), 3)
        romad = round(RoMad(weights=Wt_previous, mdd=mdd), 3)
        # print('------------------ testing -----------------------')
        # print('current portfolio value : ' + str(pf_value_previous))
    ...
    plot_cpv(
        test_pf_values,
        test_pf_values_equiweight,
        portfolio_value_init_test,
        "cpv_stocks.png",
    )
    plot_wts_assigned(
        weight_vectors[-1], ticker_num, ticker_list, "wt_vector_stocks.png"
    )
